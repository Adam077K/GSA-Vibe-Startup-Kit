# Getting Started — GSA Startup Kit

Welcome to the Green Startup Academy (GSA) Startup Kit. This guide gets you building in 5 minutes.

## 1. Clone the Kit

```bash
git clone https://github.com/Adam077K/GSA-Vibe-Startup-Kit.git
cd GSA-Vibe-Startup-Kit
```

## 2. Open in Your AI Tool

Choose one:

| Tool | How to Open |
|------|-------------|
| **Claude Code** | Open the folder; CLAUDE.md loads automatically |
| **Cursor** | File → Open Folder → select the kit folder |
| **Antigravity** | Open the folder as workspace; skills load from `.agent/skills/` |

## 3. Start Building

In the chat, type:

- **English:** "Iris, I want to build [your idea]"
- **Hebrew:** "Iris, אני רוצה לבנות [הרעיון שלך]"

Iris (the orchestrator) will route your work to specialist agents.

## Slash Commands

| Command | What it does |
|---------|--------------|
| `/daily` | Morning planning — today's focus |
| `/plan [goal]` | Sprint or feature planning |
| `/ship [feature]` | Pre-deploy: review → test → deploy |
| `/audit [focus]` | Codebase health report |
| `/research [topic]` | Deep research on a topic |

## For Green Startup Academy (כפר הירוק)

This kit is built for students at Kfar Hayarok's youth startup school. You have:

- **12 specialist agents** — Iris, Morgan, Atlas, Sage, Nova, and more
- **938+ skills** — Load with `@skill-name` (e.g. `@brainstorming`)
- **Workflow bundles** — See [workflow_bundles_readme.md](../workflow_bundles_readme.md)

## Next Steps

- Read [AGENTS.md](../AGENTS.md) for the full agent roster
- See [QUICK_REFERENCE.md](QUICK_REFERENCE.md) for a cheat sheet
- Browse skills: [.agent/skills/docs/BUNDLES.md](../.agent/skills/docs/BUNDLES.md)
