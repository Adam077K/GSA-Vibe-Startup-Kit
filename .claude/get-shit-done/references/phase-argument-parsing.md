...
```bash
PHASE_INFO=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs find-phase "${PHASE}" --raw)
```
