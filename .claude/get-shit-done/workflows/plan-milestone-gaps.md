<purpose>
Create all phases necessary to close gaps identified by `/gsa:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gsa:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/gsa:audit-milestone` first.
```
...
PHASES=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs phases list)
...
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/gsa:execute-phase {N}` — if plans already exist
...
`/gsa:audit-milestone` — re-audit to verify gaps closed
`/gsa:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/gsa:plan-phase` next
</success_criteria>
