...
```bash
INIT=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs init execute-phase "${PHASE}")
```
...
# config settings can be fetched via gsa-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gsa-executor", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn gsa-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs phases list --type summaries --raw
...
Update STATE.md using gsa-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node ./.claude/get-shit-done/bin/gsa-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node ./.claude/get-shit-done/bin/gsa-tools.cjs state update-progress

# Record execution metrics
node ./.claude/get-shit-done/bin/gsa-tools.cjs state record-metric \
...
node ./.claude/get-shit-done/bin/gsa-tools.cjs state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node ./.claude/get-shit-done/bin/gsa-tools.cjs state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using gsa-tools:
...
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs requirements mark-complete ${REQ_IDS}
```
...
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gsa:execute-phase {phase}` + `/gsa:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gsa:plan-phase {Z+1}` + `/gsa:verify-work {Z}` + `/gsa:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gsa:complete-milestone` + `/gsa:verify-work` + `/gsa:add-phase` |
