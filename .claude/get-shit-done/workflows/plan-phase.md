<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates gsa-phase-researcher, gsa-planner, and gsa-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs init plan-phase "$PHASE")
```
...
**If `planning_exists` is false:** Error — run `/gsa:new-project` first.
...
```bash
PHASE_INFO=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first": Display `/gsa:discuss-phase {X}` and exit workflow.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GSA ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn gsa-phase-researcher

```bash
PHASE_DESC=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
<files_to_read>
- {context_path} (USER DECISIONS from /gsa:discuss-phase)
...
**Project skills:** Check .agent/skills/ directory (if exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
```
Task(
  prompt="First, read ./.claude/agents/gsa-phase-researcher.md for your role and instructions.\n\n" + research_prompt,
...
```bash
node ./.claude/get-shit-done/bin/gsa-tools.cjs commit-docs "docs(phase-${PHASE}): add validation strategy"
```
...
## 8. Spawn gsa-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GSA ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**Project instructions:** Read ./CLAUDE.md if exists — follow project-specific guidelines
**Project skills:** Check .agent/skills/ directory (if exists) — read SKILL.md files, plans should account for project skill rules
</planning_context>

<downstream_consumer>
Output consumed by /gsa:execute-phase. Plans need:
...
```
Task(
  prompt="First, read ./.claude/agents/gsa-planner.md for your role and instructions.\n\n" + filled_prompt,
  subagent_type="general-purpose",
  model="{planner_model}",
...
## 10. Spawn gsa-plan-checker Agent
...
**Project skills:** Check .agent/skills/ directory (if exists) — verify plans account for project skill rules
</verification_context>
...
Task(
  ...
  subagent_type="gsa-plan-checker",
  model="{checker_model}",
...
Task(
  prompt="
    ...
    4. When spawning executor agents, use subagent_type='gsa-executor' with the existing @file pattern from the workflow
    5. When spawning verifier agents, use subagent_type='gsa-verifier'
    6. Preserve the classifyHandoffIfNeeded workaround (spot-check on that specific error)
    7. Do NOT use the Skill tool or /gsa: commands
    </instructions>
  ",
  subagent_type="general-purpose",
...
- /gsa:plan-phase {X} --research — re-research first
...
- [ ] gsa-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] gsa-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] gsa-plan-checker spawned with CONTEXT.md
