...
```bash
INIT=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs init milestone-op)
```
...
```bash
CHECKER_MODEL=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs resolve-model gsa-integration-checker --raw)
```
...
node ./.claude/get-shit-done/bin/gsa-tools.cjs phases list
```
...
PHASE_INFO=$(node ./.claude/get-shit-done/bin/gsa-tools.cjs find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="gsa-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node ./.claude/get-shit-done/bin/gsa-tools.cjs summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
- /gsa:complete-milestone {version} — proceed anyway (accept tech debt)
